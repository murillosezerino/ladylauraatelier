  ...
  return (
    <main className="min-h-screen bg-[#fdfaf7] text-stone-800">
      ...
      <section className="max-w-6xl mx-auto px-6 pb-24">
        <SectionHeader label="Seleção fixa da casa" title="Bolos Festivos" desc={bolosFestivos.descricao} />
        <div className="overflow-x-auto mb-12">
          <table className="w-full text-sm bg-white rounded-2xl border border-rose-100 overflow-hidden">
            <thead className="bg-rose-50">
              <tr>{["Tamanho", "Diâmetro", "Altura", "Porções", "Preço"].map((h) => (
          ...
          {bolosFestivos.sabores.map((s) => (
            <div key={s.nome} className="scroll-card w-72 bg-white border border-rose-100 rounded-2xl overflow-hidden hover:shadow-md transition-shadow flex flex-col">
              <div className="relative">
                ...
                {s.badge === "best-seller" && (
                  <span className="absolute top-3 right-3 bg-amber-400 text-white text-[10px] font-bold uppercase tracking-wider px-2 py-1 rounded-full shadow">Best Seller</span>
                )}
              </div>
              <div className="p-5 flex flex-col flex-1">
                <h4 style={{ fontFamily: 'var(--font-cinzel)', fontSize: '1.125rem' }} className="text-stone-800 mb-2">{s.nome}</h4>
                <p className="text-stone-500 text-sm leading-relaxed mb-4 flex-1">{s.descricao}</p>
                <a href={`${wppBase}Olá!+Gostaria+de+encomendar+um+bolo+${encodeURIComponent(s.nome)}.`} target="_blank" rel="noopener noreferrer"
      ...
      <section className="bg-rose-50/50 py-24">
        <div className="max-w-6xl mx-auto px-6">
          <SectionHeader label="Bolos artísticos personalizados" title="Bolos em Buttercream" desc={bolosButtercream.descricao} />
          <div className="overflow-x-auto mb-10">
            <table className="w-full text-sm bg-white rounded-2xl border border-rose-100 overflow-hidden">
              <thead className="bg-rose-50">
                <tr>{["Tamanho", "Diâmetro", "Porções", "Preço"].map((h) => (
                  <th key={h} className="px-5 py-4 text-left label-caps text-rose-500">{h}</th>
                ))}</tr>
              </thead>
              <tbody className="divide-y divide-rose-50">
          ...
          <HorizontalScroll>
            <div className="scroll-card w-80 bg-white border border-rose-100 rounded-2xl overflow-hidden hover:shadow-md transition-shadow">
              <ImageCarousel images={["/images/bolos/macarons1.jpg", "/images/bolos/macarons2.jpg", "/images/bolos/macarons3.jpg", "/images/bolos/macarons4.jpg"]} alt="Macarons & Flores" />
              <div className="p-5">
                <h4 style={{ fontFamily: 'var(--font-cinzel)', fontSize: '1.125rem' }} className="text-stone-800 mb-2">{bolosButtercream.estilosDecoracao[0].nome}</h4>
                <p className="text-stone-500 text-sm leading-relaxed">{bolosButtercream.estilosDecoracao[0].descricao}</p>
              </div>
            </div>
            <div className="scroll-card w-80 bg-white border border-rose-100 rounded-2xl overflow-hidden hover:shadow-md transition-shadow">
              <ImageCarousel images={["/images/bolos/drip-cake1.jpg", "/images/bolos/drip-cake2.jpg", "/images/bolos/drip-cake3.jpg", "/images/bolos/drip-cake4.jpg"]} alt="Drip Cake" />
              <div className="p-5">
                <h4 style={{ fontFamily: 'var(--font-cinzel)', fontSize: '1.125rem' }} className="text-stone-800 mb-2">{bolosButtercream.estilosDecoracao[1].nome}</h4>
                <p className="text-stone-500 text-sm leading-relaxed">{bolosButtercream.estilosDecoracao[1].descricao}</p>
              </div>
            </div>
          </HorizontalScroll>
          <div className="mt-10">
            <a href={`${wppBase}Olá!+Gostaria+de+encomendar+um+bolo+em+Buttercream+personalizado.`} target="_blank" rel="noopener noreferrer"
      ...
      <section className="max-w-6xl mx-auto px-6 py-24">
        ...
        <div className="bg-white border border-rose-100 rounded-2xl overflow-hidden grid md:grid-cols-2">
          ...
          <div className="p-10 flex flex-col justify-center">
            ...
            <a href={`${wppBase}Olá!+Gostaria+de+informações+sobre+doces+para+eventos.`} target="_blank" rel="noopener noreferrer"
